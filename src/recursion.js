...
var sum = function(array) {
  ...
  var add = arrCopy.pop();
  if (add === undefined) { return 0; } // base case
  return add + sum(arrCopy);
...
var sumBelow = function(n) {
  ...
  if ( n > 0 ){
    var summed = n - 1;
    var result = summed += sumBelow(summed);
    return result;
  }
  if ( n < 0) {
    var summed = n + 1;
...
var modulo = function(x, y) {
  return ( (x - y) < y ) ? x - y : modulo((x-y), y);
};

// 12. Write a function that multiplies two numbers without using the * operator or
// Math methods.
var multiply = function(x, y) {
  // how do we handle negatives here?
  if ( y === 0 ) { return 0 }; // for zero multiplier
  if ( y === 1 ) {return x }
  if ( y === -1 ) { return -x } // for negative multiplier
  if ( y < -1 ) { return -x + multiply(x, y+1)} // for negative multipliers
...
var reverseArr = function(array) {
  ...
  return [array.pop()].concat(reverse(array));
  }
};
...
var fizzBuzz = function(n) {
  ...
  else { return [n].concat(fizzBuzz(n-1))}
};
...
key !match but is object === traverse
key matches primitive === change value
key matches object === change key and then recurseKeys


var replaceKeysInObj = function(obj, oldKey, newKey) {
  Object.keys(obj).forEach(function(key) {
    if (typeof obj[key] === 'object'){ // recursive in case value is object
      replaceKeysInObj(obj[key], oldKey, newKey);
    }
    if (key === oldKey) { // base: @ single depth, store value and change keys
      var tempStore = obj[key];
      delete obj[key];
      obj[newKey] = tempStore;
    }
   });
};
